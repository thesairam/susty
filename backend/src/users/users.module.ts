...
import { User, Post, Listing, SustyEvent, EventRsvp, UserChallenge, Donation, CommunityPost } from '../entities';
...
class UsersService {
  ...
  async getStats(userId: number) {
    const [posts, listings, rsvps, completedChallenges, donations, communityPosts] = await Promise.all([
    ...
    return {
      activity: { posts, marketplaceItems: listings, eventsAttended: rsvps, challengesCompleted: completedChallenges, donations, communityPosts },
...
@Module({
  imports: [TypeOrmModule.forFeature([User, Post, Listing, EventRsvp, UserChallenge, Donation, CommunityPost])],
